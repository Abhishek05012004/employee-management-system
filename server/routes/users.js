...
router.get("/", auth, managerAuth, async (req, res) => {
  try {
    ...
    const query = { isActive: true, adminCode: req.user.adminCode }
...
router.get("/departments", auth, async (req, res) => {
  try {
    const departments = await User.distinct("department", { isActive: true, adminCode: req.user.adminCode })
...
router.delete("/:id", auth, adminAuth, async (req, res) => {
  try {
    ...
    const deletedEmail = `${oldEmail}_deleted_${Date.now()}`
    
    user.isActive = false
    user.email = deletedEmail
    await user.save()

    // Free up email in registration requests too
    await RegistrationRequest.updateMany(
      { email: oldEmail },
      { $set: { email: deletedEmail } }
    )
...
router.get("/dashboard-stats", auth, managerAuth, async (req, res) => {
  try {
    ...
    const totalEmployees = await User.countDocuments({ isActive: true, adminCode: req.user.adminCode })
    ...
    const departmentStats = await User.aggregate([
      { $match: { isActive: true, adminCode: req.user.adminCode } },
...
router.get("/recent-attendance", auth, managerAuth, async (req, res) => {
  try {
    ...
    const recentAttendance = await Attendance.find({ date: today, adminCode: req.user.adminCode })
