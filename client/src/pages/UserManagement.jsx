...
import { Users, Plus, Search, Edit, Trash2, Eye, EyeOff, X, Mail, Lock, User, Building, Briefcase, Phone, MapPin } from "lucide-react"
  ...
  const fetchUsers = async () => {
    ...
    try {
      if (filters.status === "active") {
        ...
        const statusParam = filters.status === "pending" ? "pending" : "rejected"
        const reqRes = await API.get("/auth/registration-requests", {
          params: {
            status: statusParam,
            page: pagination.currentPage,
            limit: 10,
        ...
        let requests = reqRes.data.requests.map((req) => ({
          ...req,
          _id: req._id,
          isPending: req.status === "pending",
          isRejected: req.status === "rejected",
          isActive: false,
          employeeId: req.status === "pending" ? "Pending" : "Rejected",
          dateOfJoining: req.createdAt,
        }))
        ...
        if (filters.department) {
          requests = requests.filter((u) => u.department === filters.department)
        }

        setUsers(requests)
        setPagination({
          currentPage: reqRes.data.currentPage,
          totalPages: reqRes.data.totalPages,
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <div className="relative">
              <input
                ...
                style={{ color: "#111827", backgroundColor: "#ffffff" }}
                className="w-full pl-4 pr-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 bg-white"
          ...
          <div className="flex items-end">
            <button
              onClick={() => {
                setFilters({ search: "", department: "", status: "active" })
                setPagination({ ...pagination, currentPage: 1 })
              }}
          ...
          <div className="overflow-x-auto">
                ...
                {users.map((user) => (
                  <tr key={user._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {user.isRejected && (
                        <span
                          className="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800 max-w-[200px] inline-block truncate"
                          title={user.rejectionReason}
                        >
                          Rejected: {user.rejectionReason || "No reason"}
                        </span>
                      )}
                      {!user.isPending && !user.isRejected && (
            ...
            {users.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">No record found for this filter</p>
              </div>
            )}
          </div>
        )}
